...
\item If  there is a number $r$ so that $a_{n \mbox{\tiny$ + 1$}} = r a_{n}$ for all $n \geq k$, then $\left\{ a_{n} \right\}_{n=k}^{\infty}$ is called an \index{sequence ! geometric ! definition of} \index{geometric sequence} \textbf{geometric sequence}.  The number $r$ is called the \index{sequence ! geometric ! common ratio} \textbf{common ratio}.
...
\item  An arithmetic sequence with first term $a$ and common difference $d$ is given by \[a_{n} = a + (n-1) d, \quad n \geq 1\] \index{sequence ! arithmetic ! formula for $n^{\textrm{th}}$ term}

\vspace{-.3in}

\item  A geometric sequence with first term $a$ and common ratio $r \neq 0$ is given by  \[ a_{n} = ar^{n-1}, \quad n \geq 1\] \index{sequence ! geometric ! formula for $n^{\textrm{th}}$ term}
